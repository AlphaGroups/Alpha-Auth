...
#         return db.query(Student).filter(Student.college_id == admin.college_id).all()
...
#     # Restrict admin to their college
#     if current_user.role == RoleEnum.admin:
#         admin = db.query(Admin).filter(Admin.user_id == current_user.id).first()
#         if student.college_id != admin.college_id:
#             raise HTTPException(status_code=403, detail="Not authorized")
...
# def update_student(student_id: int, data: dict, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
#     student = db.query(Student).filter(Student.id == student_id).first()
#     if not student:
#         raise HTTPException(status_code=404, detail="Student not found")
...
async def import_students(
    ...
    Admin → auto-assigns their college_id
    Superadmin → must provide college_id in CSV
    All students will get the selected `class_name` and `section`.
    """
    # Validate role
    if current_user.role not in [RoleEnum.admin, RoleEnum.superadmin]:
        raise HTTPException(status_code=403, detail="Only admins can import students")

    # Admin → get college_id
    if current_user.role == RoleEnum.admin:
        admin = db.query(Admin).filter(Admin.user_id == current_user.id).first()
        if not admin:
            raise HTTPException(status_code=400, detail="Admin record not found")
        ...
        try:
            ...
            if current_user.role == RoleEnum.superadmin:
                ...
                if not row_college_id:
                    errors.append(f"Row {row_num}: Superadmin must provide college_id")
                    continue
                assigned_college_id = int(row_college_id)
            ...
            if email in processed_emails or student_id in processed_ids:
                duplicates.append(f"Row {row_num}: Duplicate in CSV '{student_id}'")
                continue
    ...
    if duplicates: response["duplicates"] = duplicates
